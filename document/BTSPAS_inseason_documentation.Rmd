...
If the fishery was not open or there were zero catches, a '0' should be placed in the cell. All dates should in yyyy-mm-dd format.

These three csv files should be updated weekly, and kept placed in the data folder. 
Old versions of weekly data should be kept in the *data/prior_weeks_data folder*.
It will be easiest if you copy the previous week's file to the *data/prior_weeks_data_folder* before
modifying it with the new data.
...
The inputs that need to be specified for the code to run are the 
* stat weeks to include in the analysis, 
* the year, and 
*data.directory to store the results.
...
data.directory <-file.path('data','2019_inseason') #directory for current year
```
...
then the `add.ones.at.start=TRUE` argument should be specied in
...
The weekly inseason estimate should be the mean estimate from the file/folder *Taku-FullWeek-Inseason-Wxx-Wxx-fallback--2019*'*
output to the file 'inseason-run.size.csv.  

## Dropout rate
Model the year-to-year variation in drop out (including sampling uncertainty) as binomial with $n =  50$  and $x= 11$ 
which gives mean $p_{dropout}= 0.22$ with a standard deviation of $0.05938262$. **I thought this was changed to n=52 and x=13**?
